...
//
// something to note:
// the shadow is only applied on the default material and not the
// depth material
//
//
// we will again override some shader code
// we are overriding
// node_modules/three/src/renderers/shaders/ShaderChunk/beginnormal_vertex.glsl.js

// so we are overriding `#include <beginnormal_vertex>`

// open the file, and there you will find out that you
// can use `objectNormal`
      ...
      if (child instanceof THREE.Mesh) {
        if (
          ...
          child.material.onBeforeCompile = (shader) => {
            ...
            // ok, this is what we are doing here
            // we are doing it here in between because
            // we founf out the order of these includes inside
            ...
            shader.vertexShader = shader.vertexShader.replace(
              ...
              // pay attention
              // if we define angle here
              // comment it out in next shader override
              float angle = position.z *  4.0 * cos(uTime * 0.1) * 15.0;
              // also this needs to be commented out in next override
              mat2 rotateMatrix = get2dRotateMatrix(angle);
            ...
            shader.vertexShader = shader.vertexShader.replace(
              ...
              // again my code, I tried something different (from earlier)


              // and here we are commenting out these
              // since we calculated these in previous override
              // or to be precise they are already declared here
              // float angle = position.z *  4.0 * cos(uTime * 0.1) * 15.0;
              // mat2 rotateMatrix = get2dRotateMatrix(angle);
              transformed.xy = rotateMatrix * transformed.xy;
